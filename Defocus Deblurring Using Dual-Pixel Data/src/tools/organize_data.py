...
path2src = "/home/yqchen/Data/DualPixelCanon/valid/r_view"
path2trg = "/home/yqchen/Data/DualPixelCanon/valid/r_canny"
utils.try_make_path_exists(path2trg)
fn_list = sorted(os.listdir(path2src))
for fn in fn_list:
    img = cv2.imread(os.path.join(path2src, fn))
    edge = cv2.Canny(img, 0, 200)
    path2dest = os.path.join(path2trg, fn)
    succeed = cv2.imwrite(path2dest, edge)
    print("{} to save image to {}".format("Succeed" if succeed else "Failed", path2dest))