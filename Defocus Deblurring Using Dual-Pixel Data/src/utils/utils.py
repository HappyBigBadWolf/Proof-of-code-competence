...
def inference(model, data, device):
    ...
    """
    _INFER = {}
    def add_infer_func(func):
        _INFER[func.__name__] = func
        return func
    ...
    def _inference_V2(model, data, device):
        l_view, r_view, l_canny, r_canny = data["l_view"], data["r_view"], data["l_canny"], data["r_canny"]
        assert len(l_view.shape) == len(r_view.shape) == len(l_canny.shape) == len(r_canny.shape) == 4, "Incorrect shape."
        inp = torch.cat([l_view, l_canny, r_view, r_canny], dim=1)
        inp = inp.to(device)
        out = model(inp)
        return out, 
    ...
    return _INFER["_inference_V%s" % _VERSION_](model, data, device)
...
def inference_and_cal_loss(model, data, loss_fn, device):
    ...
    """
    def _infer_and_cal_loss_V1(model, data, loss_fn, device):
        # NOTE 2021-01-22
    ...
    def _infer_and_cal_loss_V3(model, data, loss_fn, device):
        # Serve for multi-resolution model, work with _inference_V3
        out, others = inference(model, data, device)
        target = data["target"].to(device)
        loss = loss_fn(out, target)
        for idx in range(len(others)):
            trg = F.interpolate(target, size=others[idx].shape[2: ])
            loss = loss + loss_fn(others[idx], trg)
        return out, loss
    '''    
    
    return _infer_and_cal_loss_V1(model, data, loss_fn, device)
